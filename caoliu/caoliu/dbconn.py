...
class Connection(object):
    ...
    def reconnect(self):
        ...
        auto_commit = self._db_args.get('autocommit', True)
        self._db.autocommit(auto_commit)
    ...
    def insert_by_dict(self, tablename, rowdict, replace=False):
        ...
        fields = set(row[0] for row in cursor.fetchall())
        value_fields = fields.intersection(rowdict)

        if len(rowdict) > len(value_fields):
            unknown_keys = set(rowdict) - fields
            logging.error('unknown keys: %s' % ', '.join(unknown_keys))

        columns = ''
        for i in value_fields:
            columns += '`' + i + '`' + ','
        columns = columns.rstrip(',')
        value_template = ', '.join(['%s'] * len(value_fields))

        if replace:
            sql = "REPLACE INTO %s (%s) VALUES(%s)" % (tablename, columns, value_template)
        else:
            sql = "INSERT INTO %s (%s) VALUES(%s)" % (tablename, columns, value_template)

        insert_values = tuple(rowdict[key] for key in value_fields)
    ...
    def lock_execute(self, query, *parameters, **kwparameters):
        cursor = self._cursor()
        try:
            self._execute(cursor, query, parameters, kwparameters)
            return cursor.lastrowid
